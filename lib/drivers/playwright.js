...
class PlaywrightDriver {
  ...
  async evaluateWithPageEvents(expression, options = {}) {
    if (!this.target || !this.target.page) {
      throw Object.assign(new Error('尚未 resolveTarget'), { code: 'E_NO_TAB' });
    }
    ...
    const onPopup = (popup) => events.popup.push(popup.url() || '');
    const onContextPage = (newPage) => events.contextPage.push(newPage.url() || '');
    const onDialog = (dialog) => {
      events.dialog.push({ type: dialog.type(), message: dialog.message() });
      dialog.dismiss().catch(() => {});
    };
    const onFilechooser = () => { events.filechooser += 1; };
    const onDownload = () => { events.download += 1; };
    ...
    try {
      ...
      const settleMs = options.settleMs == null ? 1200 : options.settleMs;
      if (settleMs > 0) {
