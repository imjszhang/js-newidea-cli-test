...
async function main(argv = process.argv.slice(2)) {
  ...
  try {
    ...
    if (cmd === 'cta') {
      ...
      const ctaOptions = {};
      if (opts.fulltextLang) ctaOptions.fulltextLang = opts.fulltextLang;
      if (opts.cnRefs) ctaOptions.cnRefs = opts.cnRefs;
      if (opts.dialogAction) ctaOptions.dialogAction = opts.dialogAction;
      const wantsDestructiveConfirm = name === '生成全文'
        ? (ctaOptions.dialogAction || 'confirm') !== 'cancel'
        : DESTRUCTIVE_CTAS.has(name);
      if (wantsDestructiveConfirm && !opts.confirm) {
        console.error(`"${name}" 是破坏性操作（会离开 ${pageProfile.routeLabel}），请加 --confirm 再试。`);
        return 3;
      }
      const res = await sess.callApi('clickCta', Object.keys(ctaOptions).length ? [name, ctaOptions] : [name]);
      renderResult(res, opts);
      return res && res.ok ? 0 : 1;
    }
