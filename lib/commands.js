...
const COMMANDS = {
  doctor:      { kind: 'special', help: '连通性 + 注入 + probe + state 汇总', pages: ['outline', 'home', 'proofread'] },
  ...
  cta:         { kind: 'special', help: '点击全局 CTA（返回首页/下载提纲/生成全文）；全文支持弹窗选项', pages: ['outline'] },
  ...
  'select-section': { kind: 'call', api: 'selectSection', argSpec: [{ name: 'paragraphIndex', required: true }], toArgs: (_o, [index]) => [index], help: '选中 proofread 页某段（按 0-based paragraphIndex）', pages: ['proofread'] },
  'edit-section': { kind: 'call', api: 'enterEditMode', argSpec: [{ name: 'paragraphIndex', required: true }], toArgs: (_o, [index]) => [index], help: '进入 proofread 页某段编辑态（先选中，再点编辑提示）', pages: ['proofread'] },
  'editor-button': { kind: 'call', api: 'clickEditorButton', argSpec: [{ name: 'name', required: true }], toArgs: (_o, [name]) => [name], help: '点击 proofread 编辑态按钮（确认校对/AI 改写/一键重写/一键复制/替换原文）', pages: ['proofread'] },
  'rewrite-prompt': { kind: 'call', api: 'setRewritePrompt', argSpec: [{ name: 'text', required: true }], toArgs: (_o, [text]) => [text], help: '设置 proofread 页 AI 改写额外需求输入框', pages: ['proofread'] },
  state:       { kind: 'call', api: 'state', argSpec: [], toArgs: () => [], help: '读取当前页面运行态', pages: ['outline', 'home', 'proofread'] },
  probe:       { kind: 'call', api: 'probe', argSpec: [], toArgs: () => [], help: '读取页面指纹与关键控件状态', pages: ['outline', 'home', 'proofread'] },
};
...
function parseArgv(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (a === '--fulltext-lang') opts.fulltextLang = argv[++i];
    else if (a.startsWith('--fulltext-lang=')) opts.fulltextLang = a.slice('--fulltext-lang='.length);
    else if (a === '--cn-refs') opts.cnRefs = argv[++i];
    else if (a.startsWith('--cn-refs=')) opts.cnRefs = a.slice('--cn-refs='.length);
    else if (a === '--dialog-action') opts.dialogAction = argv[++i];
    else if (a.startsWith('--dialog-action=')) opts.dialogAction = a.slice('--dialog-action='.length);
...
function printHelp() {
  ...
  lines.push(
    ...
    '  --fulltext-lang   生成全文弹窗语言（zh | en | 中文 | English）',
    '  --cn-refs         生成全文弹窗中文文献（yes | no | 是 | 否）',
    '  --dialog-action   生成全文弹窗操作（confirm | cancel）',
    '  -v, --verbose     打印细节',
    '  -h, --help        显示帮助',
  );
