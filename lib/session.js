...
class Session {
  ...
  async callApi(method, args = []) {
    ...
    if (this.pageProfile.name === 'home' && method === 'clickPrimary' && typeof this.driver.evaluateWithPageEvents === 'function') {
      let observed;
      try {
        ...
        observed = {
          result: {
            ...
            data: {
              ...
              afterUrl: currentUrl,
              afterPath: currentUrl ? new URL(currentUrl).pathname : '',
              urlChanged: !!currentUrl && !currentUrl.includes(this.pageProfile.targetUrlFragment),
              pathChanged: !!currentUrl && new URL(currentUrl).pathname !== this.pageProfile.routeLabel,
