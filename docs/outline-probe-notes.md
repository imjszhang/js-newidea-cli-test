...
### 6.1 `生成全文` 确认框（2026-04-21 补充）

点击底部 `生成全文` 后，当前站点会出现页面内确认框，而不是浏览器原生 `dialog`。

已观测到的确认框元素：

- 标题：`确认生成全文`
- 提示文案：`是否已完成提纲修改，继续生成全文？`
- 操作按钮：`取消` / `确定`

已观测到的选项组：

- 稳定存在：全文生成语言 `中文 / English`
- 非稳定存在：中文文献 `是 / 否`

说明：

- “中文文献”这一组在不同页面版本上并不总是出现
- 选中态不是 aria/radio 语义，而是靠滑块位置与文字颜色变化体现
- bridge 现已支持在点击 `生成全文` 后继续驱动该确认框，并允许选择：
  - 语言
  - 中文文献（若该组存在）
  - 最终动作：`confirm` / `cancel`
- 当前已验证：点击 `确定` 后页面会从 `/outline` 跳转到 `/proofread`
